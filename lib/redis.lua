...
function connect(self)
    if not self.connected then
        ...
        if rds and not self.connect_refused then
            ...
            local ok, err = rds:connect(host, cfg.port)
            if not ok then
            ...
            if cfg.password ~= '' then
                local ok, err = rds:auth(cfg.password)
                if not ok then
                    self.connect_refused = true
                    log('failed auth redis: ' .. cfg.password 
                        .. ':' .. ' - ' .. (err and err or ''))
                end
            end
...
function hget(self, myhash, field)
    local rds, res = self.rds
    if rds then
        if not self.connected then
            self:connect()
...
function hset(self, myhash, field, value)
    local rds, res = self.rds
    if rds then
        if not self.connected then
            self:connect()
        ...
        if self.connected then
            res = rds:hset(myhash, field, value)
            if res and res == 'OK' then
                res = true
            end
            self:dg('hset')
